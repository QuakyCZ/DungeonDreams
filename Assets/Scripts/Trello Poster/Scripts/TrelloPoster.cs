	...
	public class TrelloPoster : ScriptableObject
	{
		...
		private void CheckWebRequestStatusAndDispose(UnityWebRequest webRequest, string errorMessage = "Web Request Error: ")
		{
			if (string.IsNullOrEmpty(webRequest.error))
			{
				...
				GameObject.FindGameObjectWithTag("ErrorText").GetComponent<Text>().text = "Zpráva byla odeslána.";
			}
			else
			{
				Debug.LogError(errorMessage + webRequest.downloadHandler.text);
				GameObject.FindGameObjectWithTag("ErrorText").GetComponent<Text>().text = "Při odesílání nastala chyba. Zkuste to znovu.";
			}
			webRequest.Dispose();
		}
