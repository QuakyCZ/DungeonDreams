...
public class ClonedTile {
    ...
    public List<ClonedTile> GetNeighbours() {
        ...
        if( tile.x + 1 < width && tile.y + 1 < height && tiles[x,y+1].isWalkable && tiles[x+1,y].isWalkable) {
            nb.Add( tiles[x + 1, y + 1] );
        }
        if ( tile.x + 1 < width && tile.y - 1 > 0 && tiles[x,y-1].isWalkable && tiles[x+1,y].isWalkable) {
            nb.Add( tiles[x + 1, y - 1] );
        }
        if ( tile.x - 1 > 0 && tile.y + 1 < height && tiles[x,y+1].isWalkable && tiles[x-1,y].isWalkable) {
            nb.Add( tiles[x - 1, y + 1] );
        }
        if ( tile.x - 1 > 0 && tile.y - 1 > 0 && tiles[x-1,y].isWalkable && tiles[x,y-1].isWalkable) {
            nb.Add( tiles[x - 1, y - 1] );
        }
